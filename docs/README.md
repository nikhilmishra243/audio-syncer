...
### Step 1: Make the startup script executable (First time only)
...
**Windows:**
No setup needed - just double-click `start.bat`
...
## 📋 Optional: Audio Source Selection

For best results, run this once to select your audio output device:

```bash
# Activate venv first (only if not using start.sh)
...
This helps ensure system audio (not microphone) is captured correctly.
...
> **💡 Pro Tip:** Use the provided `start.sh` (Linux/macOS) or `start.bat` (Windows) scripts - they handle all setup automatically! No need to manually create venv, activate it, or install dependencies.
...
**Easiest way - Use startup script (recommended):**
```bash
...
3. **Python dependencies missing:**
   ```bash
   ./start.sh  # Auto-installs dependencies
   ```
...
```
.
├── start.sh               # Auto-setup launcher (Linux/macOS) - RUN THIS!
├── start.bat              # Auto-setup launcher (Windows) - RUN THIS!
├── launcher.py            # Control panel server
...
ExecStart=/path/to/audio-streamer/start.sh
Restart=always
...
| Module not found errors | Use `start.sh`/`start.bat` - auto-installs dependencies |
| python3 command not found | Install Python 3, then run startup script |
...
| Virtual environment issues | Delete `venv/` folder and run `start.sh` again |
...
**Then:**
1. Open admin panel → Click "Start Server"
2. Share user page with listeners
3. Play music and enjoy! 🎵
...
Edit `launcher.py` to personalize the admin panel:
...
- Original Windows-only version earlier
- Cross-platform enhancements for wider compatibility
- BlackHole audio driver for macOS support
- PulseAudio/PipeWire teams for Linux audio infrastructure
- Flask and asyncio communities
...
**Nikhil Mishra** - Java Spring Boot Developer
